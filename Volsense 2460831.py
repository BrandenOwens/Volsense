...
# === PREDICTED DIRECTION COLORS ===
predicted_colors = []
for i in range(len(raw_pressures)):
    if i == 0:
        predicted_colors.append("gray")
    elif raw_pressures[i] > raw_pressures[i - 1]:
...
if save_to_desktop:
    ...
    filename = f"Volsense_Prediction_{selected_date.strftime('%Y-%m-%d')}.txt"
...
def show_chart():
    ...
    # === Add price delta label with arrows ===
    for i, date in enumerate(recent_data.index):
        ...
        label = f"{'+' if delta >= 0 else ''}{delta:.2f} {arrow}"
        plt.text(
            x=i,
            y=volsense[i] + 2,
            s=label,
